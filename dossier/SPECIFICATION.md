# Universal Project Dossier Specification

Octon Mini version: 4.0.0
...
- **[Recommended]** Introduced by this specification to close a reusable gap.
...
An Octon Mini setup session is likewise an orchestration input, not dossier or
...
Apparent reference gaps and the specification response:
...
1. **Conceptual artifact type** — a reusable information contract such as
   ...
   `dossier/artifact-types.json` in this specification and copied into a generated
...
- **Recommended format:** Markdown navigation plus a manifest conforming to
  the current Octon Mini Context Pack manifest schema when the type is applicable
...
This table is the complete Octon Mini source mapping. `Profile` means the
