...
TRANSACTION = load_module(
    "pb_collaboration_transaction",
    SKILL_ROOT / "assets/templates/core/.agent/scripts/pb_transaction.py.tmpl",
)
PACKAGE = load_module("pb_collaboration_decisions", SCRIPT_ROOT / "package_project.py")
...
def build_plan(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if project.get("schema_version") != "harness.project.v4":
        raise ValueError("target does not use the collaboration-v2 Project Blueprint contract")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Plan/apply progressive collaboration assessment")
