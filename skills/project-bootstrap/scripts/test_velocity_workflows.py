#!/usr/bin/env python3
"""Integration tests for the Project Blueprint 4.0 velocity workflows."""
...
def import_transaction(project: Path):
    path = project / ".agent/scripts/pb_transaction.py"
    spec = importlib.util.spec_from_file_location(
        f"pb_velocity_transaction_{hash(project)}", path
    )
...
class VelocityWorkflowTests(unittest.TestCase):
    ...
    def test_guided_first_task_lifecycle_stale_plan_and_interruption_recovery(self) -> None:
        with tempfile.TemporaryDirectory(prefix="pbv-velocity-init-") as temporary:
            area = Path(temporary)
            ...
            resumed = run([str(project / "pb"), "work", "resume"], project)
            ...
            rolled_back = run(
                [str(project / "pb"), "transaction", "rollback", "--receipt", str(receipt)],
                project,
            )
            ...
            recovered = run(
                [str(project / "pb"), "transaction", "recover", "--pending", str(pending_path)],
                project,
            )
            ...
            refreshed = run([str(project / "pb"), "maintain", "refresh", "--apply"], project)
            ...
            check = run([str(project / "pb"), "check", "--json"], project)
    ...
    def test_detector_archetypes_are_read_only_in_clean_and_dirty_repositories(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="pbv-velocity-detect-") as temporary:
            area = Path(temporary)
    ...
    def test_collaboration_bands_and_concurrency_remain_profile_independent(self) -> None:
        with tempfile.TemporaryDirectory(prefix="pbv-velocity-collaboration-") as temporary:
            area = Path(temporary)
            project = area / "project"
            result = run(
                [
            ...
            self.assertEqual(load(project / ".project-blueprint-origin.json")["profile"], "standard")

    def test_semantic_adoption_preserves_established_and_dirty_content(self) -> None:
        with tempfile.TemporaryDirectory(prefix="pbv-velocity-adoption-") as temporary:
            area = Path(temporary)
