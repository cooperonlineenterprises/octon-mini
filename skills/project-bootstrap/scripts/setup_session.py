...
SESSION_SCHEMA = "project-blueprint.setup-session.v1"
ANSWER_SCHEMA = "project-blueprint.setup-answers.v1"
...
def blueprint_root() -> Path:
    ...
    for candidate in candidates:
        if (
            ...
            return candidate
    raise SetupError("Blueprint source with the setup question catalog is unavailable")


def source_version() -> str:
    value = (blueprint_root() / "VERSION").read_text(encoding="utf-8").strip()
    if re.fullmatch(r"[0-9]+\.[0-9]+\.[0-9]+", value) is None:
        raise SetupError("Blueprint source VERSION is malformed")
    return value


def catalog_path() -> Path:
    return blueprint_root() / "shared/source-contracts/setup-questions.json"
...
def validate_catalog(value: Any) -> dict[str, Any]:
    ...
    if (
        value.get("schema_version") != "project-blueprint.setup-question-catalog.v1"
...
def origin_observation(target: Path) -> tuple[str, str | None, list[str], list[str]]:
    origin_path = target / ".project-blueprint-origin.json"
    if not origin_path.exists():
        return "absent", None, [], ["no .project-blueprint-origin.json was observed"]
    ...
    version = value.get("blueprint_version") if isinstance(value, dict) else None
    schema = value.get("schema_version") if isinstance(value, dict) else None
    if (
        not isinstance(version, str)
        or re.fullmatch(r"[0-9]+\.[0-9]+\.[0-9]+", version) is None
        or not isinstance(schema, str)
        or not schema.startswith("project-blueprint.origin.v")
    ):
        return "invalid", None, [], ["origin provenance lacks a valid schema or Blueprint version"]
    installed: list[str] = []
    ...
    if packages_path.is_file() and not packages_path.is_symlink():
        try:
            ...
            installed = []
    return "valid", version, installed, [f"valid {schema} provenance records Blueprint {version}"]
...
def detect_mode(target: Path) -> tuple[str, list[str], dict[str, Any]]:
    ...
    elif any(name in {"pb", ".agent", "project-dossier"} for name in relevant):
        mode = "ambiguous"
        evidence.append("Blueprint-like generated markers exist without valid origin provenance")
    else:
        mode = "adoption"
        evidence.append("established content exists without Blueprint origin provenance")
...
def initial_observations(
    ...
    provenance_value = dict(blueprint)
    ...
    return [
        ...
        state_record(questions["setup.blueprint-provenance"], "answered", provenance_value, "inspection", observed_evidence("filesystem:Blueprint-origin")),
    ]
        ...
        records.extend(
            [
                ...
                },
                {
                    ...
                    "evidence_refs": ["project-blueprint:profile-manifest"],
                },
            ]
        )
...
def work_completion_assessment(session: dict[str, Any], states: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    if "small-team-git-portfolio" not in blueprint.get("installed_packages", []):
...
def create_session(mode: str, target: Path) -> dict[str, Any]:
    ...
    if not target.exists():
        ...
        observed_target = target
    detected, evidence, blueprint = detect_mode(observed_target)
    ...
    session: dict[str, Any] = {
        ...
        "question_states": initial_observations(catalog, observed_target, detected, evidence, blueprint),
...
def validate_session_shape(value: Any, catalog: dict[str, Any]) -> dict[str, Any]:
    required = {
        ...
        "target_identity", "target_revision", "target_fingerprint", "governing_instruction_fingerprint", "blueprint", "question_catalog", "selected_profile",
...
def current_state_mismatches(session: dict[str, Any]) -> list[str]:
    ...
    mismatches: list[str] = []
    detected, _, blueprint = detect_mode(target)
    ...
    for key in ("source_version", "candidate_version", "installed_version", "provenance_status", "installed_packages"):
        if blueprint.get(key) != session["blueprint"].get(key):
            mismatches.append(f"Blueprint {key}")
...
def question_batch(session: dict[str, Any], catalog: dict[str, Any], limit: int) -> dict[str, Any]:
    ...
    return {
        "schema_version": "project-blueprint.setup-question-batch.v1",
        "permission_grant": False,
        "session_id": session["session_id"],
...
def add_setup_parser(commands: argparse._SubParsersAction[argparse.ArgumentParser], mode: str) -> None:
    setup = commands.add_parser("setup", help="read-only guided question generation with optional explicit external session output")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Shared Project Blueprint guided setup session engine")
