...
def load_detector():
    ...
    spec = importlib.util.spec_from_file_location("octon_mini_detector", path)
    if spec is None or spec.loader is None:
...
def classify_existing(target: Path) -> dict[str, Any]:
    indicators = {
        ...
        "octon_mini_origin": (target / ".octon-mini-origin.json").is_file(),
        "legacy_project_blueprint_origin": (target / ".project-blueprint-origin.json").is_file(),
...
def origin_summary(target: Path, scaffolder: Any) -> dict[str, Any] | None:
    path = target / ".octon-mini-origin.json"
    if not path.is_file():
        return None
    try:
    ...
    return {
        ...
        "octon_mini_version": value.get("octon_mini_version"),
...
def build_plan(
    ...
    *,
    ...
    plan = {
        "schema_version": "octon-mini.bootstrap.adoption-proposal.v1",
        "authority": (
        ...
        "required_sequence": [
            ...
            "Run target checks plus Octon Mini mutation tests on the exact result.",
        ...
        "non_transfer_rules": [
            ...
            "Do not treat absent Octon Mini paths as proof of a project gap until functional equivalents are reviewed.",
        ],
    }
