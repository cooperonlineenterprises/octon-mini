#!/usr/bin/env python3
"""Install a collision-safe Octon Mini Project Bootstrap skill snapshot."""
...
def default_destination() -> Path:
    ...
    return base / "skills" / "octon-mini-project-bootstrap"
...
def octon_mini_source_root(skill_source: Path) -> Path:
    candidates = (
        skill_source.parents[1],
        skill_source / "assets" / "octon-mini-source",
    )
...
def main() -> int:
    ...
    if args.dry_run:
        ...
        return 0
    try:
        repository_root = octon_mini_source_root(source)
    except ValueError as error:
        print(f"ERROR: {error}", file=sys.stderr)
        return 2
    with tempfile.TemporaryDirectory(
        prefix=".octon-mini-project-bootstrap-install-", dir=destination.parent
    ) as temporary:
        ...
        shutil.copytree(
            ...
            ignore=shutil.ignore_patterns(
                ".DS_Store", "__pycache__", "*.pyc", "octon-mini-source"
            ),
        )
        bundle = stage / "assets" / "octon-mini-source"
    ...
    print("Installed Octon Mini Project Bootstrap skill snapshot.")
    print(
        "The staged package, bundled Octon Mini source, and profile builds passed; "
